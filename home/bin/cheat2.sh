...
HISTORY_FILE="$HOME/.config/cheatsheets/.cheat_history"

export EDITOR=nvim

# Enable readline for command history
set -o emacs  # Use emacs-style key bindings for readline
...
save_to_history() {
    local cmd="$1"
    if [ -n "$cmd" ] && [ "$cmd" != "q" ] && [ "$cmd" != "quit" ] && [ "$cmd" != "exit" ]; then
...
# Enhanced display function that uses glow for formatting and bat-like navigation
enhanced_display() {
    local file="$1"
    local title="$2"
    ...
    # Check available tools and use the best combination
    if command -v glow >/dev/null 2>&1 && command -v bat >/dev/null 2>&1; then
        # Use glow for markdown formatting, then pipe to bat for navigation
        echo -e "${BLUE}Displaying: ${YELLOW}$title${NC}"
        echo
        bat "$file" --paging=always --language=markdown --style=plain | glow -p 
        ...
        echo -e "${BLUE}Displaying: ${YELLOW}$title${NC}"
        echo
        bat --paging=always --language=markdown "$file"
    else
        # Fallback to less
        echo -e "${BLUE}Displaying: ${YELLOW}$title${NC}"
        echo
...
show_help() {
    ...
    echo -e "  ${GREEN}show <topic>${NC}   - Display cheatsheet"
    ...
    echo -e "  ${GREEN}clear${NC} or ${GREEN}c${NC}     - Clear screen"
    ...
    echo -e "  ${CYAN}↑/↓ arrows${NC}     - Browse command history"
    echo -e "  ${CYAN}Ctrl+R${NC}         - Reverse search history"
    echo
}
...
search_cheatsheets() {
    ...
    if [ -s "$TEMP_FILE" ]; then
        enhanced_display "$TEMP_FILE" "Search Results"
    else
...
show_history() {
    if [ -f "$HISTORY_FILE" ]; then
        ...
        if command -v bat >/dev/null 2>&1; then
            cat "$HISTORY_FILE" | nl -ba | bat --paging=always --language=bash --style=numbers
        else
            cat "$HISTORY_FILE" | nl -ba | less
        fi
    else
...
fzf_cheatsheet() {
    ...
    if command -v glow >/dev/null 2>&1; then
        preview_cmd="glow '$CHEAT_DIR/{}.md'"
    elif command -v bat >/dev/null 2>&1; then
        preview_cmd="bat --style=plain '$CHEAT_DIR/{}.md'"
...
fzf_command_search() {
    ...
    if [ -s "$temp_search" ]; then
        enhanced_display "$temp_search" "All Cheatsheets"
    fi
...
show_cheatsheet() {
    ...
    if [ -f "$file" ]; then
        enhanced_display "$file" "$topic Cheatsheet"
    else
...
# Function to read input with history support
read_with_history() {
    local prompt="$1"
    local input
    
    # Use readline with history support and print prompt with -e flag
    read -e -p "$(echo -e "$prompt")" input
...
# Interactive mode with enhanced history support
interactive_mode() {
    # Load command history into current bash session
    if [ -f "$HISTORY_FILE" ]; then
    ...
    echo -e "${CYAN}Use ↑/↓ arrows for command history, Ctrl+R for reverse search${NC}"
    echo
    
    while true; do
        # Use readline for input with history support
        input=$(read_with_history "${CYAN}cheat> ${NC}")
    ...
    case "$1" in
        ...
        "show")
            if [ -n "$2" ]; then
                ...
                echo -e "${RED}Usage: $0 show <topic>${NC}"
            fi
            ;;
        *)
            # Treat as search query
